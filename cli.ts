...
export const IMAGE_GEN_DEFAULT_MODEL = "nano-banana-2";
...
export type ImageGenAlias = keyof typeof MODEL_ALIASES;
...
export function resolveModelSpecifier(specifier: string): ModelResolution {
  ...
  const aliasedModel = MODEL_ALIASES[trimmed as ImageGenAlias];
  if (aliasedModel) {
    return { alias: trimmed, model: aliasedModel };
  }

  if (isVendorModelId(trimmed)) {
    return { alias: null, model: trimmed };
  }

  throw new ImageGenCliError(
    "validation_error",
    `Unknown model "${trimmed}". Use --list-models to see built-in aliases or pass a full vendor/slug OpenRouter model ID.`,
  );
}
...
export function validateInput(raw: Record<string, unknown>, env: NodeJS.ProcessEnv = process.env): ValidatedImageGenInput {
  ...
  const { alias, model } = resolveModelSpecifier(modelSpecifier);
...
// 3. OpenRouter client and decoding
...
export function extractApiErrorMessage(parsedBody: unknown, rawText: string, status: number): string {
  ...
  return trimmed || `OpenRouter request failed with status ${status}.`;
}
...
export async function requestOpenRouter(
  ...
  try {
    ...
    if (!response.ok) {
      const message = extractApiErrorMessage(parsedBody, rawText, response.status);
...
export async function generateImages(
  ...
  while (generatedImages.length < input.n && attempts < input.n) {
    ...
    const response = await requestOpenRouter(buildRequestBody(input, referenceImages, requestCount), {
      env,
      fetchImpl: options.fetchImpl,
    });
    const batch = decodeGeneratedImages(response);
    if (batch.length === 0) {
      throw new ImageGenCliError("api_error", "OpenRouter returned no generated images.");
    }
...
Aliases:
  nano-banana-2   -> ${MODEL_ALIASES["nano-banana-2"]}   default, text+image
  nano-banana-pro -> ${MODEL_ALIASES["nano-banana-pro"]}   high-fidelity, text-heavy scenes
  gpt-image       -> ${MODEL_ALIASES["gpt-image"]}   reasoning-heavy image generation
...
Usage:
  ...
  image-gen --help | --version | --list-models

Aliases:
  nano-banana-2   -> ${MODEL_ALIASES["nano-banana-2"]} (default)
  nano-banana-pro -> ${MODEL_ALIASES["nano-banana-pro"]}
  gpt-image       -> ${MODEL_ALIASES["gpt-image"]}
...
JSON input fields:
  ...
  size              string hint such as 512, 1024, 2K, 4K
  n                 integer 1-4
...
Output shape:
  {"ok":true,"path":"/abs/primary.png","paths":["/abs/primary.png"],"model":"...","alias":"nano-banana-2","provider":"openrouter","bytes":1234,"elapsed_ms":900,"cost_usd":0.0032,"prompt":"..."}
  {"ok":false,"error":"...","code":"auth_error"}

Environment:
  OPENROUTER_API_KEY      required; also loaded from the package-local .env
  OPENROUTER_BASE_URL     optional API base override
  IMAGE_GEN_DEFAULT_MODEL optional alias or vendor/slug default
